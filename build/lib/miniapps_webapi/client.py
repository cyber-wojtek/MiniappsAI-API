...
class MiniAppsClient:
    ...
    def fetch_csrf(self) -> str:
        """
        GET /auth/csrf
        Seeds the __Host-miniapps.x-csrf-token cookie and returns the
        csrfToken value that must be sent as the x-csrf-token header.
        Call this once before any POST on a fresh session.
        """
...
--------
  # One-shot chat (uses saved cookies if present)
    python -m miniapps_webapi.client --slug claude-37 --message "What is the capital of France?"

  # Continue a conversation
    python -m miniapps_webapi.client --slug claude-37 \\
